...
const SUPPORTED_IOS_14_3_BROWSERS = [
  'chrome',
  'firefox',
  'safari'
];
...
/**
 * Check whether the current browser is running on iOS 14.3 or greater, i.e., a
 * version of iOS that supports WebRTC in third party browsers.
 * @returns {boolean}
 */
function isIosWithThirdPartyWebRTCSupport() {
  const iosMajorMinorMatch = navigator.userAgent.match(
    // Match the part of the string that starts with "(iPhone;", then find the
    // version from there. Two example user agents we need to account for:
    // - "... (iPhone; CPU iPhone OS 14_4 ..."
    // - "... (iPhone; CPU OS 14_4_1 ..."
    /\(iP(hone|ad|od); .*? OS (\d+_\d+)/
  );

  const userAgentIosVersion = iosMajorMinorMatch ? iosMajorMinorMatch[2] : '0';

  // Convert the iosMajorMinorMatch[2] from, e.g., "14_7" to 14.7
  const iosMajorMinorVersion = parseFloat(
    userAgentIosVersion.replace('_', '.')
  );

  // Starting with iOS 14.3, third party apps can use WebRTC
  return iosMajorMinorVersion >= 14.3;
}
...
function isSupported() {
  ...
  const supportedMobileBrowsers = /android/.test(
    navigator.userAgent.toLowerCase()
  )
    ? SUPPORTED_ANDROID_BROWSERS
    : isIosWithThirdPartyWebRTCSupport()
      ? SUPPORTED_IOS_14_3_BROWSERS
      : SUPPORTED_IOS_BROWSERS;
